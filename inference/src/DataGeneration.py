...
class DataGenerator(keras.utils.Sequence):
    ...
    def __data_generation(self, DSM_IDs_temp, PAN_IDs_temp, LABEL_IDs_temp):
        ...
        for i in range(len(DSM_IDs_temp)):
            '''
            # read tif file & to numpy array
            arr_dsm = rasterio.open(DSM_IDs_temp[i]).read()
            arr_pan = rasterio.open(PAN_IDs_temp[i]).read()
            arr_label = rasterio.open(LABEL_IDs_temp[i]).read()
            if arr_dsm.shape == (1, 256, 256):
                # move channel
                DSM_out.append(np.moveaxis(arr_dsm,0,2))
                PAN_out.append(np.moveaxis(arr_pan,0,2))
                LABEL_out.append(np.moveaxis(arr_label,0,2))
            '''
            # move channel
