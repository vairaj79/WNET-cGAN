...
class Data:
    ...
    def get_data(self, start=0, num=10, as_arr=True, random=False):
        ...
        for i in idx:
            DSM_out.append(np.moveaxis(rasterio.open(self.DSM[i]).read(),0,2).astype(np.float32))
            PAN_out.append(np.moveaxis(rasterio.open(self.PAN[i]).read(),0,2).astype(np.float32))
            LABEL_out.append(np.moveaxis(rasterio.open(self.LABEL[i]).read(),0,2).astype(np.float32))
